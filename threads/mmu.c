...
{
	int idx = PDX(va); // 가상주소를 인덱스로 변환
	if (pdp)
	{
		uint64_t *pte = (uint64_t *)pdp[idx]; // 페이지 테이블 엔트리를 가져옴
		if (!((uint64_t)pte & PTE_P))
		{ // PTE_P가 0이면 페이지테이블 엔트리가 없는 것
			if (create)
			{ // create가 참이면 페이지 테이블 엔트리를 생성
		...
		/*마지막으로 ptov() 매크로를 사용하여 PTE_ADDR(pdp[idx]) + 8 * PTX(va)에서 계산한 물리 주소를 가상 주소로 변환합니다. 이 변환된 가상 주소는 해당 가상 주소 va와 연결된 페이지 프레임의 물리 주소를 가리키는 페이지 테이블 엔트리의 가상 주소입니다.

다시 말해, 이 함수는 페이지 디렉토리에서 pdp와 va로 주어진 가상 주소에 해당하는 페이지 테이블 엔트리를 검색하고, 이 페이지 테이블 엔트리가 가리키는 물리 주소를 가상 주소로 변환하여 반환합니다. 이를 통해 가상 주소 va가 가리키는 메모리 위치에 접근할 수 있게 됩니다.*/
